...
libname = {'win32': f'w{bits}'}
os_bits = {'w32': 'x86/TofDaqDll.dll', 'w64': "x64/TofDaqDll.dll"}
os.chdir(str(os.path.dirname(os.path.join(os.path.dirname(os.path.abspath(__file__)), os_bits[libname[sys.platform]]))))
libTofDaq = ct.cdll.LoadLibrary(str(os.path.join(os.path.dirname(os.path.abspath(__file__)),
...
class TSharedMemoryDesc(ct.Structure):
    _fields_ = [("nbrSamples", ct.c_int),
                ...
                ("enableVarNbrMemories", ct.c_bool),
                ("massCalDim", ct.c_ubyte),
                ("dummy1", ct.c_ubyte),
                ("dummy2", ct.c_ubyte),
                ("nbrSteps", ct.c_int),
...
def TwManualContinueNeeded():
    if libTofDaq._TwManualContinueNeeded():
        return True
    else:
        return False
...
def TwSaveIniFile(iniFilename):
    libTofDaq._TwSaveIniFile.argtypes = [ct.c_char_p]
...
def TwGetDaqParameterIntRef(Parameter, pValue):
    if isinstance(pValue, type(ct.byref(ct.c_int32()))):
        libTofDaq._TwGetDaqParameterInt.argtypes = [ct.c_char_p, ct.POINTER(ct.c_int32)]
    else:
...
def TwGetDaqParameterBoolRef(Parameter, pValue):
    if isinstance(pValue, type(ct.byref(ct.c_bool()))):
        libTofDaq._TwGetDaqParameterInt.argtypes = [ct.c_char_p, ct.POINTER(ct.c_bool)]
    else:
        libTofDaq._TwGetDaqParameterBoolRef.argtypes = [ct.c_char_p, ndpointer(ct.c_bool, shape=1)]
    return libTofDaq._TwGetDaqParameterBoolRef(Parameter, pValue)


def TwGetDaqParameterFloatRef(Parameter, pValue):
    if isinstance(pValue, type(ct.byref(ct.c_float()))):
        libTofDaq._TwGetDaqParameterInt.argtypes = [ct.c_char_p, ct.POINTER(ct.c_float)]
    else:
...
def TwGetDaqParameterInt64Ref(Parameter, pValue):
    if isinstance(pValue, type(ct.byref(ct.c_int64()))):
        libTofDaq._TwGetDaqParameterInt.argtypes = [ct.c_char_p, ct.POINTER(ct.c_int64)]
    else:
...
def TwGetDaqParameterDoubleRef(Parameter, pValue):
    print(type(pValue))
    if isinstance(pValue, type(ct.byref(ct.c_double()))):
        libTofDaq._TwGetDaqParameterInt.argtypes = [ct.c_char_p, ct.POINTER(ct.c_double)]
    else:
...
def TwSetMassCalib(mode, nbrParams, p, nbrPoints, mass, tof, weight):
    libTofDaq._TwSetMassCalib.argtypes = [ct.c_int, ct.c_int, ndpointer(np.float64), ct.c_int, ndpointer(np.float64),
...
def TwGetPeakParameters(PeakIndex, PeakPar=None):
    libTofDaq._TwGetPeakParameters.argtypes = [ct.POINTER(TPeakPar), ct.c_int]
    if PeakPar != None:
...
def TwWaitForNewData(Timeout, ShMemDescriptor, ShMem, WaitForEventReset):
    if ShMem == None:
        ...
        libTofDaq._TwWaitForNewData.argtypes = [ct.c_int, ct.POINTER(TSharedMemoryDesc),
                                                ct.POINTER(TSharedMemoryPointer), ct.c_bool]
...
def TwUpdateUserDataPy(location, data):
    dataBuffer = np.zeros((len(data),), dtype=np.float64)
...
def TwGetRegUserDataTargetRange(location, elementIndex, minValue, maxValue):
    libTofDaq._TwGetRegUserDataTargetRange.argtypes = [ct.c_char_p, ct.c_int, ndpointer(np.float64, shape=1),
                                                       ndpointer(np.float64, shape=1)]
    return libTofDaq._TwGetRegUserDataTargetRange(location, elementIndex, minValue, maxValue)


def TwGetRegUserDataTargetRangePy(location: str, element_index: int) -> Tuple[
        TwRetVal, Union[Tuple[None, None], Tuple[float, float]]]:
    min_value = np.ndarray(shape=(1,), dtype=np.float64)
    max_value = np.ndarray(shape=(1,), dtype=np.float64)
    rv = TwGetRegUserDataTargetRange(location.encode(), element_index, min_value, max_value)
    if rv == TwRetVal.TwSuccess.value:
        return TwRetVal(rv), (float(min_value[0]), float(max_value[0]))
    else:
        return TwRetVal(rv), (None, None)
...
def TwQueryRegUserDataSizePy(location: str) -> Tuple[TwRetVal, Union[int, None]]:
    nbr_elements = np.ndarray(shape=(1,), dtype=np.int32)
    rv = TwQueryRegUserDataSize(location.encode(), nbr_elements)
    if rv == TwRetVal.TwSuccess.value:
        return TwRetVal(rv), int(nbr_elements[0])
    else:
        return TwRetVal(rv), None
...
def TwGetRegUserDataDescPy(location: bytes) -> Union[None, List[bytes]]:
    desc = []
...
def TwGetRegUserDataSources(arrayLength, location, nbrElements, dsType):
    if location is None and nbrElements is None and dsType is None:
        libTofDaq._TwGetRegUserDataSources.argtypes = [ndpointer(np.int32, shape=1), ct.c_void_p, ct.c_void_p,
                                                       ct.c_void_p]
    else:
        libTofDaq._TwGetRegUserDataSources.argtypes = [ndpointer(np.int32, shape=1), ct.c_char_p, ndpointer(np.int32),
                                                       ndpointer(np.int32)]
    return libTofDaq._TwGetRegUserDataSources(arrayLength, location, nbrElements, dsType)


def TwGetRegUserDataSourcesPy() -> Union[None, List[Tuple[bytes, int, int]]]:
    sources = []
...
def TwTpsConnect2Py(ip: str = "localhost", tpsType: Union[0 | 1] = 0) -> TwRetVal:
    ip_c = ip.encode()
    return TwRetVal(libTofDaq._TwTpsConnect2(ip_c, tpsType))
...
def TwTpsLoadSetFile2(setFile, rcBlackList, blackListLength, rcWhiteList, whiteListLength):
    libTofDaq._TwTpsLoadSetFile2.argtypes = [ct.c_char_p,
                                             ct.c_void_p if rcBlackList is None else ndpointer(np.int32,
                                                                                               shape=blackListLength),
                                             ct.c_int,
                                             ct.c_void_p if rcWhiteList is None else ndpointer(np.int32,
                                                                                               shape=whiteListLength),
                                             ct.c_int]
    return libTofDaq._TwTpsLoadSetFile2(setFile, rcBlackList, blackListLength, rcWhiteList, whiteListLength)
...
def TwTpsGetModuleLimits(moduleCode, minLimit, maxLimit):
    libTofDaq._TwTpsGetModuleLimits.argtypes = [ct.c_int, ndpointer(np.float64, shape=1),
                                                ndpointer(np.float64, shape=1)]
    return libTofDaq._TwTpsGetModuleLimits(moduleCode, minLimit, maxLimit)


def TwTpsGetModuleLimitsPy(module_code: int) -> Tuple[TwRetVal, Tuple[Union[float, None], Union[float, None]]]:
    min_lim = np.ndarray(shape=(1,), dtype=np.float64)
    max_lim = np.ndarray(shape=(1,), dtype=np.float64)
    rv = TwTpsGetModuleLimits(module_code, minLimit=min_lim, maxLimit=max_lim)
    if rv == TwRetVal.TwSuccess.value:
        return TwRetVal(rv), (float(min_lim[0]), float(max_lim[0]))
    else:
        return TwRetVal(rv), (None, None)


def TwTpsChangeIonMode(ionMode):
    libTofDaq._TwTpsChangeIonMode.argtypes = [ct.c_int]
    return libTofDaq._TwTpsChangeIonMode(ionMode)
...
if TOFDAQDLL_REV >= 1019:
    def TwConfigureForSingleIonMeasurement(nbrBits, negativeSignal):
        libTofDaq._TwConfigureForSingleIonMeasurement.argtypes = [ndpointer(np.int32, shape=1),
                                                                  ndpointer(np.int8, shape=1)]
...
if TOFDAQDLL_REV >= 1083:
    ...
    def TwTpsGetModulePropertiesPy(moduleCode):
        ...
        if (TwTpsGetModuleProperties(moduleCode, prop, label) == TwSuccess):
            ...
            return {'label': labelStr, 'hasMonitor': (properties & 0x1 != 0), 'isSettable': (properties & 0x2 != 0),
                    'isTrigger': (properties & 0x4 != 0)}
        else:
            return {}
...
if TOFDAQDLL_REV >= 1343:
    def TwGenerateSegmentProfilesFromEventData(nbrProfiles, startMass, endMass, bufIndex, data, startEndInSamples,
                                               tofSpec):
        libTofDaq._TwGenerateSegmentProfilesFromEventData.argtypes = [ct.c_int,
                                                                      ndpointer(np.float64, shape=nbrProfiles),
                                                                      ndpointer(np.float64, shape=nbrProfiles),
                                                                      ct.c_int, ndpointer(np.float32), ct.c_bool,
                                                                      ct.c_void_p if tofSpec is None else ndpointer(
                                                                          np.float32)]
        return libTofDaq._TwGenerateSegmentProfilesFromEventData(nbrProfiles, startMass, endMass, bufIndex, data,
                                                                 startEndInSamples, tofSpec)

#TOFWERK_DAQ_API TwRetVal TwGetMassCalibFromShMem(int* mode, int* nbrParams, double* p, int* nbrPoints, double* mass, double* tof, double* weight, char* label, double* sis, int segmentIndex, int bufIndex);
if TOFDAQDLL_REV >= 1411:
    def TwGetMassCalibFromShMem(mode, nbrParams, p, nbrPoints, mass, tof, weight, label, sis, segmentIndex, bufIndex):
        libTofDaq._TwGetMassCalibFromShMem.argtypes = [ndpointer(np.int32, shape=1), ndpointer(np.int32, shape=1),
                                                       ct.c_void_p if p is None else ndpointer(np.float64,
                                                                                               shape=nbrParams),
                                                       ndpointer(np.int32, shape=1),
                                                       ct.c_void_p if mass is None else ndpointer(np.float64,
                                                                                                  shape=nbrPoints),
                                                       ct.c_void_p if tof is None else ndpointer(np.float64,
                                                                                                 shape=nbrPoints),
                                                       ct.c_void_p if weight is None else ndpointer(np.float64,
                                                                                                    shape=nbrPoints),
                                                       ct.c_void_p if label is None else ct.c_char_p,
                                                       ct.c_void_p if sis is None else ndpointer(np.float64, shape=1),
                                                       ct.c_int, ct.c_int]
        return libTofDaq._TwGetMassCalibFromShMem(mode, nbrParams, p, nbrPoints, mass, tof, weight, label, sis,
                                                  segmentIndex, bufIndex)

#TOFWERK_DAQ_API TwRetVal TwSetMassCalibInShMem(int mode, int nbrParams, double* p, int nbrPoints, double* mass, double* tof, double* weight, char* label, double* sis, int segmentIndex, int bufIndex);
if TOFDAQDLL_REV >= 1411:
    def TwSetMassCalibInShMem(mode, nbrParams, p, nbrPoints, mass, tof, weight, label, sis, segmentIndex, bufIndex):
        libTofDaq._TwSetMassCalibInShMem.argtypes = [ct.c_int, ct.c_int,
                                                     ct.c_void_p if p is None else ndpointer(np.float64,
                                                                                             shape=nbrParams),
                                                     ct.c_int,
                                                     ct.c_void_p if mass is None else ndpointer(np.float64,
                                                     ...
                                                     ct.c_void_p if sis is None else ndpointer(np.float64, shape=1),
                                                     ct.c_int, ct.c_int]
        return libTofDaq._TwSetMassCalibInShMem(mode, nbrParams, p, nbrPoints, mass, tof, weight, label, sis,
                                                segmentIndex, bufIndex)

#bool TwRecorderNeedsRestart(void)
if TOFDAQDLL_REV >= 1492:
    def TwRecorderNeedsRestart():
        libTofDaq._TwRecorderNeedsRestart.restype = ct.c_bool
        return libTofDaq._TwRecorderNeedsRestart()

#TOFWERK_DAQ_API TwRetVal TwTpsSaveRdsToSetFileRc(char* setFile, char* rdsName, int nbrElements, int* rdsIndex, double* rdsVals);
if TOFDAQDLL_REV >= 1391:
    def TwTpsSaveRdsToSetFileRc(setFile, rdsName, nbrElements, rdsIndex, rdsVals):
        libTofDaq._TwTpsSaveRdsToSetFileRc.argtypes = [ct.c_char_p, ct.c_char_p, ct.c_int,
                                                       ndpointer(np.int32, shape=nbrElements),
                                                       ndpointer(np.float64, shape=nbrElements)]
        return libTofDaq._TwTpsSaveRdsToSetFileRc(setFile, rdsName, nbrElements, rdsIndex, rdsVals)

#TOFWERK_DAQ_API TwRetVal TwTpsLoadSetFile3(char* setFile, int* rcBlackList, int rcBlackListLength, int* rcWhiteList, int rcWhiteListLength, char* rdsWhiteList, int rdsWhiteListLength);
if TOFDAQDLL_REV >= 1391:
    def TwTpsLoadSetFile3(setFile, rcBlackList, blackListLength, rcWhiteList, whiteListLength, rdsWhiteList,
                          rdsWhiteListLength):
        libTofDaq._TwTpsLoadSetFile3.argtypes = [ct.c_char_p,
                                                 ct.c_void_p if rcBlackList is None else ndpointer(np.int32,
                                                                                                   shape=blackListLength),
                                                 ct.c_int,
                                                 ct.c_void_p if rcWhiteList is None else ndpointer(np.int32,
                                                                                                   shape=whiteListLength),
                                                 ct.c_int,
                                                 ct.c_void_p if rdsWhiteList is None else ct.c_char_p,
                                                 ct.c_int]
        return libTofDaq._TwTpsLoadSetFile3(setFile, rcBlackList, blackListLength, rcWhiteList, whiteListLength, rdsWhiteList, rdsWhiteListLength)
    

#TwRetVal TwStoreFile(char* fileLocation, char* vfsLocation);
if TOFDAQDLL_REV >= 1685:
    def TwStoreFile(fileLocation, vfsLocation):
        libTofDaq._TwStoreFile.argtypes = [ct.c_char_p, ct.c_char_p]
        return libTofDaq._TwStoreFile(fileLocation, vfsLocation)
